...
const PostPanel = () => {
    ...
    const submitUpdateForm = (e) => {
        ...
        axios.post(`http://127.0.0.1:8000/api/postUpdate/${updateId}`, data)
            ...
            .catch(() => {
                alert("Internal Error ... Please Check PostPanel - submitUpdateForm", e);
    ...
    const submitPostForm = async (e) => {
        ...
        if (item.length != 0) {
            ...
            axios.post('http://127.0.0.1:8000/api/postStore', data,{ headers: {"Authorization" : `Bearer ${user.token}`}} )
                .then((e) => {
                    console.log(e);
                    alert(e.data);
                ...
                .catch(() => {
                    alert("Internal Error ... Please Check PostPanel - submitPostForm", e);
    ...
    const getData = (e) => {
        axios.get(`http://127.0.0.1:8000/api/userPost/${user.userId}`)
            ...
            .catch(() => {
                alert("Internal Error ... Please Check PostPanel - getData", e);
    ...
    const getCategory = (e) => {
        axios.get(`http://127.0.0.1:8000/api/categoryView`)
            ...
            .catch(() => {
                alert("Internal Error ... Please Check PostPanel - GetCategory", e);
    ...
    const deletePost = (id,userid) => {
        ...
        axios.delete(`http://127.0.0.1:8000/api/postDelete/${id}`)
            ...
            .catch((e) => {
                alert("Internal Error ... Please Check Post Panel - DeletePost", e);
            ...
            <div>
                {status === "" ? "" : (status ?
                    <form onSubmit={submitPostForm}>
                        ...
                        <h3 className='underline text-xl	font-bold	uppercase text-center indent-3 m-2'>All Posts</h3>

                        <div className='mt-8  ml-5  grid grid-cols-4 gap-6'>
                            {
                                post.map((e) => {
                                    return (<>
                                        <div className=" bg-slate-300 w-64 h-50 rounded-md overflow-hidden shadow-xl">
